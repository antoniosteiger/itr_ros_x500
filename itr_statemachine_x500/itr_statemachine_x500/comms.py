...
class Comms():
    ...
    def _init_subs(self):
        ...
        self._make_sub(VehicleStatus, "/fmu/out/vehicle_status_v1", self._status_callback)
        self._make_sub(VehicleCommandAck, "/fmu/out/vehicle_command_ack", self._cmd_ack_callback)
    ...
    def _cmd_ack_callback(self, msg: VehicleCommandAck):       
        ...
        if self.debug:
            if self.cmd_is_success():
                self.node.get_logger().info(f"[Comms:cmd] Command {self._cmd_ack_id} succeeded.")
            else:
                self.node.get_logger().info(f"[Comms:cmd] Command {self._cmd_ack_id} FAILED.") 
    ...
    def _make_pub(self, type, topic: str):
        self._pubs[topic] = self.node.create_publisher(
            type,
            topic,
            QOS_PROFILE_PX4_PUB
        )
    ...
    def _send_cmd(self, msg: VehicleCommand):
        ...
        if self.debug:
            self.node.get_logger().info(f"[Comms:cmd] Command {msg.command} has been sent.")
